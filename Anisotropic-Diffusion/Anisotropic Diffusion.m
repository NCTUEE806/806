function diff_im = anisodiff2D(im, num_iter, kappa,delta_t, option)
...
hN = [0 0 0; 0 -1 0; 0 1 0];
hS = [0 1 0; 0 -1 0; 0 0 0];
hE = [0 0 0; 1 -1 0; 0 0 0];
hW = [0 0 0; 0 -1 1; 0 0 0];
hNE = [0 0 0; 0 -1 0; 1 0 0];
hSE = [1 0 0; 0 -1 0; 0 0 0];
hSW = [0 0 1; 0 -1 0; 0 0 0];
hNW = [0 0 0; 0 -1 0; 0 0 1];
% [m n] = size(im)
% [Fx Fy]=gradient(diff_im);
% gradientImg=round(sqrt(Fx.^2+Fy.^2));
% NumGradient=ceil(max(max(gradientImg)));
% h=zeros(NumGradient,1);
% kappa=30;
% for level=0:NumGradient
%     h(level+1,1)=sum(sum(gradientImg==level));
% end
% sum1=0;
% for level=0:NumGradient
%     sum1=sum1+h(level+1,1);
%     if sum1>=0.9*m*n
%         kappa=30;
%         sum1=0;
%         break;
